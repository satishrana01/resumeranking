...
def skillSetListMatchedWithJD(resume, jdTxt,skill_weightage,programming_skill_from_input=""):
    ...
    jd_corpus = list(set(jdTxt.lower().split()))
...
def JDkeywordMatch(JDText, ResumeText, Weightage):
    ...
    JD_Keyword_Matched = [keys for keys in jd_WordList if keys in keywords_list] 
    Resume_Keyword_Matched = [keys for keys in resume_WordList if keys in keywords_list]
...
def nonTechSkillSetListMatchedWithJD(resume, jdTxt,rank,soft_skill_from_input=""):
    ...
    finalResult = {}
    global soft_skills
    if bool(soft_skill_from_input and soft_skill_from_input.strip()):
        soft_skills = soft_skill_from_input.split(",")
